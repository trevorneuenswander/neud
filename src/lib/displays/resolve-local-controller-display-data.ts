import { resolveMergedReserveStatusLabelOrUnknown } from "@/lib/bag/reserve-status";
import {
  hasCanonicalBid,
  normalizeDisplayBid,
  resolveCanonicalBidAmount,
} from "@/lib/displays/display-bid-normalization";
...
function mergePhotoUrls(
  ...
  const add = (value: string | OfflinePhotoReference | null | undefined) => {
    ...
    if (!resolved || !isUsablePhotoUrl(resolved) || seen.has(resolved)) return;
    seen.add(resolved);
  ...
  return filterUsablePhotoUrls(ordered);
}
...
export function resolveLocalControllerDisplayData(input: {
  ...
  const photos = mergePhotoUrls(submittedLot ?? {}, datasetLot);
  ...
  const reserveStatus = resolveMergedReserveStatusLabelOrUnknown(
  ...
  return {
    ...
    auctionDisplay: {
      ...
      reserveStatus: currentLot.reserveStatus ?? "",
      biddingPrice,
      currencies,
      photos,
    },
