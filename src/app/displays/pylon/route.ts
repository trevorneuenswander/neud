...
async function isPylonEnabled(): Promise<boolean> {
  if (!shouldUseLocalData()) {
    return true;
  }

  try {
    const response = await fetch(`${getLocalApiBaseUrl()}/api/displays/pylon/enabled`, {
      cache: "no-store",
    });
...
function loadPylonHtml(): string {
  const htmlPath = path.join(process.cwd(), "public", "displays", "pylon", "index.html");
...
export async function GET(request: Request) {
  ...
  const enabled = await isPylonEnabled();
  ...
  if (src) {
    output = output.replace(
      "const DEFAULT_ENDPOINT = '/api/displays/pylon/data';",
      `const DEFAULT_ENDPOINT = ${JSON.stringify(src)};`,
    );
  } else if (previewMode) {
    output = output.replace(
      "const DEFAULT_ENDPOINT = '/api/displays/pylon/data';",
      "const DEFAULT_ENDPOINT = '/api/displays/pylon/data?preview=1';",
    );
  }
  if (poll) {
    output = output.replace(
