...
const DISPLAY_KEY = "new-ticker-v1";
...
export async function GET(request: Request) {
  ...
  if (src) {
    output = output.replace(
      "let ENDPOINT = params.get(\"src\") || null;",
      `let ENDPOINT = ${JSON.stringify(src)};`,
    );
  }
  if (poll) {
    output = output.replace(
      "const POLL_MS = Math.max(250, +(params.get(\"poll\") || 1000));",
      `const POLL_MS = Math.max(250, +(${JSON.stringify(poll)}));`,
    );
  }
