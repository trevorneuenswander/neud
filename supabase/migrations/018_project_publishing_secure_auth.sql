-- =============================================================================
-- NEUD Alpha v0.1.1 Slice 2.1: secure cloud publishing authorization
-- =============================================================================
--
-- Replaces service_role-only publishing RPCs with authenticated, membership-aware
-- RPCs that enforce auth.uid() and hosted project authorization.
--
-- Dependency: 016_project_publishing_foundation.sql, 017_project_publishing_rpcs.sql
-- Do NOT edit 017 in place — this migration supersedes grants and function bodies.
-- =============================================================================
...
alter table public.project_publisher_leases
  add column if not exists acquired_by_user_id uuid
    references auth.users (id)
    on delete set null;

alter table public.project_canonical_snapshots
  add column if not exists published_by_user_id uuid
    references auth.users (id)
    on delete set null;

-- -----------------------------------------------------------------------------
-- Publishing authorization helpers
-- -----------------------------------------------------------------------------

create or replace function public.can_publish_project(p_project_id uuid)
returns boolean
language sql
security definer
stable
...
create or replace function public.can_manage_project_publishing(p_project_id uuid)
returns boolean
language sql
security definer
stable
...
-- -----------------------------------------------------------------------------
-- Hosted project registration (desktop ↔ cloud identity alignment)
-- -----------------------------------------------------------------------------
...
begin
  ...
  if not public.is_platform_admin() then
    return jsonb_build_object(
      'ok', false,
      'code', 'forbidden',
      'message', 'Only platform administrators may register new hosted projects.'
    );
  end if;
...
-- -----------------------------------------------------------------------------
-- Membership-aware RLS read policies
-- -----------------------------------------------------------------------------

drop policy if exists project_publishing_settings_select on public.project_publishing_settings;
create policy project_publishing_settings_select
  on public.project_publishing_settings
  for select
  to authenticated
  using (public.is_project_member(project_id));

drop policy if exists project_canonical_snapshots_select on public.project_canonical_snapshots;
create policy project_canonical_snapshots_select
  on public.project_canonical_snapshots
  for select
  to authenticated
  using (public.is_project_member(project_id));

drop policy if exists project_publisher_leases_select on public.project_publisher_leases;
create policy project_publisher_leases_select
  on public.project_publisher_leases
  for select
  to authenticated
  using (public.is_project_member(project_id));

-- -----------------------------------------------------------------------------
-- Secure publishing RPCs (authenticated callers only)
-- -----------------------------------------------------------------------------
...
begin
  ...
  if not public.can_publish_project(p_project_id) then
    return jsonb_build_object(
      'ok', false,
      'code', 'forbidden',
      'message', 'Your project role cannot acquire a publisher lease.'
    );
  end if;
...
begin
  ...
  if not public.can_publish_project(p_project_id) then
    return jsonb_build_object(
      'ok', false,
      'code', 'forbidden',
      'message', 'Your project role cannot renew a publisher lease.'
    );
  end if;
  ...
  update public.project_publisher_leases
  set
    acquired_by_user_id = coalesce(acquired_by_user_id, v_user_id),
    last_heartbeat_at = v_now,
    lease_expires_at = v_expires,
    updated_at = v_now
  where project_id = p_project_id;
...
begin
  ...
  if not public.can_manage_project_publishing(p_project_id) then
    return jsonb_build_object(
      'ok', false,
      'code', 'forbidden',
      'message', 'Your project role cannot change online publishing settings.'
    );
  end if;
...
-- -----------------------------------------------------------------------------
-- Revoke service_role execution; grant authenticated only
-- -----------------------------------------------------------------------------
