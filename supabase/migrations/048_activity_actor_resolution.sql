...
-- Resolve actor_display_name from profiles for user-attributed activity events.

create or replace function public.resolve_activity_actor_display_name(p_user_id uuid)
returns text
language sql
stable
security definer
...
create or replace function public.activity_events_set_actor_display_name()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if new.user_id is not null
    and nullif(btrim(coalesce(new.actor_display_name, '')), '') is null then
    new.actor_display_name := public.resolve_activity_actor_display_name(new.user_id);
  end if;
  return new;
end;
$$;

drop trigger if exists activity_events_actor_display_name on public.activity_events;
create trigger activity_events_actor_display_name
  before insert or update on public.activity_events
  for each row execute function public.activity_events_set_actor_display_name();
...
revoke all on function public.resolve_activity_actor_display_name(uuid) from public;
revoke execute on function public.resolve_activity_actor_display_name(uuid) from anon;
revoke execute on function public.resolve_activity_actor_display_name(uuid) from service_role;
grant execute on function public.resolve_activity_actor_display_name(uuid) to authenticated;
