...
test("0.1.3 formats as Alpha 0.1.3", () => {
  assert.equal(versionApi.getDisplayVersion("0.1.3"), "Alpha 0.1.3");
});
...
test("missing runtime data does not render Alpha undefined", () => {
  assert.equal(versionApi.getDisplayVersion(""), "Alpha 0.1.3");
  assert.equal(versionApi.getDisplayVersion("   "), "Alpha 0.1.3");
  assert.ok(!versionApi.getDisplayVersion("").includes("undefined"));
  assert.ok(!versionApi.getDisplayVersion("").includes("null"));
});

test("canonical package metadata stays at 0.1.3 in root and desktop packages", () => {
  const rootPkg = readJson("package.json");
  const desktopPkg = readJson("desktop/package.json");

  assert.equal(rootPkg.version, "0.1.3");
  assert.equal(desktopPkg.version, "0.1.3");
