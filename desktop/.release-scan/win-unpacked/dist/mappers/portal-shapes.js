...
exports.toPortalDisplay = toPortalDisplay;
exports.toPortalProject = toPortalProject;
exports.toPortalDataEngine = toPortalDataEngine;
exports.toPortalEngineStatus = toPortalEngineStatus;
exports.toPortalScraperSettings = toPortalScraperSettings;
exports.toPortalScraperSource = toPortalScraperSource;
exports.toPortalSnapshot = toPortalSnapshot;
exports.toPortalLog = toPortalLog;
function toPortalDisplay(display, localApiBaseUrl) {
    ...
    return {
        ...
        display_type: typeof settings.displayType === "string" ? settings.displayType : "custom",
        url,
        width: display.displayWidth,
        height: display.displayHeight,
        background: typeof settings.background === "string" ? settings.background : null,
...
function toPortalProject(project, ownerId = "00000000-0000-0000-0000-000000000000") {
    return {
        id: project.id,
...
function toPortalDataEngine(source) {
    const executionMode = source.config.execution_mode ?? "local-desktop";
    return {
        id: source.id,
...
function toPortalEngineStatus(sourceId, status, createdAt, options = {}) {
    if (!status) {
        return {
            engine_id: sourceId,
...
function toPortalScraperSettings(sourceId, settings, createdAt) {
    if (!settings)
        return null;
    return {
        engine_id: sourceId,
...
function toPortalScraperSource(source) {
    return {
        id: source.id,
...
function toPortalSnapshot(snapshot) {
    return {
