...
exports.migrateLegacyAppSettings = migrateLegacyAppSettings;
...
function migrateLegacyAppSettings(db) {
    const marker = db
        .prepare("SELECT value_json FROM app_settings WHERE key = ?")
        .get(LEGACY_SETTINGS_MIGRATION_KEY);
    if (marker) {
        try {
    ...
    db.transaction(() => {
        for (const [legacyKey, currentKey] of Object.entries(LEGACY_SETTING_KEY_MAP)) {
           ...
           VALUES (?, ?, datetime('now'))`).run(currentKey, legacyRow.value_json);
            }
            db.prepare("DELETE FROM app_settings WHERE key = ?").run(legacyKey);
            updated += 1;
        }
        db.prepare(`UPDATE teams SET name = ?, description = ?
       WHERE name IN ('HMG', 'NEUD') AND description LIKE 'Default %team%'`).run("Hildreth Media Group", "Default team for Hildreth Media Group within NEUD.");
        db.prepare(`INSERT INTO app_settings (key, value_json, updated_at)
       VALUES (?, ?, datetime('now'))
       ON CONFLICT(key) DO UPDATE SET
         value_json = excluded.value_json,
         updated_at = excluded.updated_at`).run(LEGACY_SETTINGS_MIGRATION_KEY, JSON.stringify("done"));
    });
    if (updated > 0) {
