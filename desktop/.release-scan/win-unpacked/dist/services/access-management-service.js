...
exports.AccessManagementService = void 0;
...
class AccessManagementService {
    ...
    recordActivity;
    supabaseInvite;
    constructor(authorization, users, teams, teamMemberships, projectMemberships, projectTeams, projects, invitations, recordActivity, supabaseInvite = null) {
        ...
        this.supabaseInvite = supabaseInvite;
    }
    getDirectory(actorUserId) {
        ...
        const users = this.users
            ...
            teamMemberships: this.teamMemberships
                .listForUser(user.id)
                .filter((membership) => context.isPlatformOwner || adminTeamIds.has(membership.teamId)),
    ...
    updateTeam(actorUserId, teamId, input) {
        const context = this.authorization.getAuthorizationContext(actorUserId);
        if (!context?.canManageTeams) {
        ...
        const identityChanged = (input.name !== undefined && input.name.trim() !== existing.name) ||
    ...
    setProjectTeams(actorUserId, input) {
        const context = this.authorization.getAuthorizationContext(actorUserId);
        if (!context?.isPlatformOwner) {
    ...
    assignProjectToTeam(actorUserId, input) {
        const context = this.authorization.getAuthorizationContext(actorUserId);
        if (!context?.isPlatformOwner) {
        ...
        const nextTeamIds = input.removeAssignments === false
    ...
    inviteUser(actorUserId, input) {
        ...
        if (input.role !== "admin" && projectIds.length === 0) {
            throw new Error(input.role === "operator"
    ...
    applyInvitationMemberships(userId, invitationId) {
        ...
        for (const projectId of invitation.projectIds) {
            this.projectMemberships.upsert({
                ...
                teamId: invitation.teamId,
                userId,
                accessRole: invitation.teamRole,
                createdByUserId: invitation.createdByUserId,
            });
        }
    }
    upsertTeamMember(actorUserId, input) {
        this.authorization.assertCanManageTeam(actorUserId, input.teamId);
        const context = this.authorization.getAuthorizationContext(actorUserId);
        if (!context) {
            throw new Error("You do not have permission to manage this team.");
    ...
    removeTeamMember(actorUserId, teamId, userId) {
    ...
    setProjectAssignment(actorUserId, input) {
        this.authorization.assertCanManageProjectForTeam(actorUserId, input.projectId, input.teamId);
        const context = this.authorization.getAuthorizationContext(actorUserId);
    ...
    removeProjectAssignment(actorUserId, projectId, teamId, userId) {
    ...
    removeProjectAccessPath(actorUserId, input) {
        const project = this.projects.getById(input.projectId);
        if (!project) {
            throw new Error("Project not found.");
        }
        ...
        const matchingPath = paths.find((path) => path.type === input.pathType && path.teamId === input.teamId);
        if (!matchingPath) {
    ...
    setUserActive(actorUserId, targetUserId, isActive) {
        const context = this.authorization.getAuthorizationContext(actorUserId);
        if (!context?.canManageUsersAndAccess) {
        ...
        if (!context.isPlatformOwner) {
            const actorTeams = new Set(context.teamMemberships
    ...
    getUserDetails(actorUserId, targetUserId) {
    ...
    resolveAssignedProjectNames(userId, adminTeamIds, context) {
        const names = new Set();
...
exports.AccessManagementService = AccessManagementService;
...
function summarizeProjectAccess(paths, projectId, projectTeams, teams) {
    if (paths.length === 0) {
...
function pathToProjectRole(path) {
    switch (path.type) {
        case "owner":
            return "Owner";
        case "team_admin":
...
function pathToAccessSource(path) {
    switch (path.type) {
        case "owner":
            return "Owner";
        case "team_admin":
