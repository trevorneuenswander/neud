...
exports.DeveloperToolsService = exports.ProjectCodeMigrationService = void 0;
...
const html_display_runtime_adapters_1 = require("../displays/html-display-runtime-adapters");
...
const display_code_validation_service_1 = require("./display-code-validation-service");
const scraper_code_validation_service_1 = require("./scraper-code-validation-service");
const project_permissions_1 = require("../projects/project-permissions");
const application_roles_1 = require("../auth/application-roles");
const revision_labels_1 = require("../developer-tools/revision-labels");
...
class ProjectCodeMigrationService {
    settings;
    projects;
    displays;
    displayCode;
    ...
    storage;
    repoRoot;
    constructor(settings, projects, displays, displayCode, scraperCode, revisions, storage, repoRoot) {
    ...
    ensureProjectMigrated(projectId, actorId) {
    ...
    seedScraper(projectId, actorId) {
        if (this.scraperCode.get(projectId)) {
            return;
        }
        const source = templates_1.DEFAULT_SCRAPER_TEMPLATE;
        ...
        this.revisions.create({
            ...
            sourceHash: (0, project_scraper_code_repository_1.hashSource)(source),
    ...
    seedDisplays(projectId, actorId) {
        ...
        for (const display of rows) {
            ...
            const html = bundled?.html ?? templates_1.BLANK_DISPLAY_HTML;
            const css = bundled?.css ?? templates_1.BLANK_DISPLAY_CSS;
            const javascript = bundled?.javascript ?? templates_1.BLANK_DISPLAY_JAVASCRIPT;
            const revisionId = (0, project_scraper_code_repository_1.createRevisionId)();
            ...
            this.revisions.create({
                ...
                sourceHash: (0, project_scraper_code_repository_1.hashSource)(`${html}\n${css}\n${javascript}`),
                validationStatus: "valid",
                createdBy: actorId,
    ...
    readBundledDisplay(displayKey) {
        const bundledPath = path_1.default.join(this.repoRoot, "public", "displays", displayKey, "index.html");
        if (!fs_1.default.existsSync(bundledPath)) {
            return null;
        }
        const html = fs_1.default.readFileSync(bundledPath, "utf8");
        return {
            html,
            css: "",
            javascript: "",
...
exports.ProjectCodeMigrationService = ProjectCodeMigrationService;
class DeveloperToolsService {
    ...
    onDisplayCreated;
    displaySyncHooks;
    constructor(projects, displays, scraperCode, displayCode, revisions, validationLogs, storage, migration, auth, memberships, dataSources, engineManager, recordActivity = null, localApiBaseUrl = null, onDisplayDeleted, onDisplayDuplicated, onDisplayUnarchived, onDisplayCreated, displaySyncHooks = {}) {
        this.projects = projects;
        this.displays = displays;
        this.scraperCode = scraperCode;
        this.displayCode = displayCode;
        ...
        this.onDisplayDeleted = onDisplayDeleted;
        this.onDisplayDuplicated = onDisplayDuplicated;
        this.onDisplayUnarchived = onDisplayUnarchived;
        this.onDisplayCreated = onDisplayCreated;
        this.displaySyncHooks = displaySyncHooks;
    }
    assertDeveloperToolsAccess(projectId) {
    ...
    assertProjectViewAccess(project) {
        const user = this.auth.getAuthenticatedUser();
        if (!user) {
            throw new Error("Sign in to your NEUD account to continue.");
        }
    ...
    resolveDisplayRecord(projectId, displayIdOrKey) {
        return (this.displays.getById(displayIdOrKey) ??
            this.displays.listByProject(projectId).find((row) => row.projectId === projectId &&
                (row.id === displayIdOrKey || row.displayKey === displayIdOrKey)) ??
            null);
    }
    getScraperBundle(projectSlug) {
    ...
    getScraperDeveloperContext(projectSlug, engineId) {
        const project = this.requireProject(projectSlug);
        ...
        const adapterName = typeof engine?.config?.adapter === "string" ? engine.config.adapter : null;
        const runningPaths = (0, revision_labels_1.resolveRunningRuntimeFilePaths)(adapterName);
    ...
    saveScraperDraft(projectSlug, input) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const row = this.scraperCode.get(project.id);
        if (!row) {
        ...
        if (input.baseRevisionId &&
            ...
            throw new Error("This code was updated by another user. Review the newer version before saving.");
        }
    ...
    validateScraperDraft(projectSlug, source) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const row = this.scraperCode.get(project.id);
        if (!row) {
        ...
        const result = (0, scraper_code_validation_service_1.validateScraperSource)(draft);
    ...
    async publishScraper(projectSlug, input) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const validation = (0, scraper_code_validation_service_1.validateScraperSource)(input.source);
        if (!validation.ok) {
            throw new Error("Scraper code must pass validation before publishing.");
        }
        const revisionName = (0, revision_labels_1.normalizeRevisionName)(input.revisionName ?? input.message);
        const changeNote = (0, revision_labels_1.normalizeChangeNote)(input.changeNote);
        const message = (0, revision_labels_1.buildRevisionMessage)({
            revisionName,
            changeNote,
        ...
        this.revisions.create({
            ...
            revisionName,
            changeNote,
            sourceHash: (0, project_scraper_code_repository_1.hashSource)(input.source),
            validationStatus: "valid",
            createdBy: actor.userId,
            message,
            metadata: { storageRevisionId: revisionId },
        ...
        try {
            ...
            this.recordProjectActivity(project, {
    ...
    listDisplayVersionSummaries(projectSlug) {
        const project = this.requireProject(projectSlug);
    ...
    listArchivedDisplays(projectSlug) {
        const project = this.requireProject(projectSlug);
        this.assertDeveloperToolsAccess(project.id);
        this.migration.ensureProjectMigrated(project.id, this.auth.getAuthenticatedUser()?.userId ?? "system");
        return this.displays
            .listByProject(project.id)
            .map((display) => {
            const code = this.displayCode.getByDisplayId(display.id);
    ...
    getDisplaySource(projectSlug, displayIdOrKey) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        this.migration.ensureProjectMigrated(project.id, actor.userId);
        ...
        return {
            ...
            isDirty: (code.draftHtml ?? "") !== (published?.html ?? "") ||
    ...
    saveDisplayDraft(projectSlug, displayId, input) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
    ...
    validateDisplayDraft(projectSlug, displayId, input) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const code = this.displayCode.getByDisplayId(displayId);
        if (!code) {
        ...
        const result = (0, display_code_validation_service_1.validateDisplaySource)({
            slug: code.slug,
    ...
    publishDisplay(projectSlug, displayId, input) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const code = this.displayCode.getByDisplayId(displayId);
        if (!code) {
        ...
        const validation = (0, display_code_validation_service_1.validateDisplaySource)({
            slug: code.slug,
            html: input.html,
            css: input.css,
        ...
        const nextSourceHash = (0, project_scraper_code_repository_1.hashSource)(`${input.html}\n${input.css}\n${input.javascript}`);
        const published = this.storage.readDisplayPublished(project.id, displayId);
        if (published) {
            const currentHash = (0, project_scraper_code_repository_1.hashSource)(`${published.html}\n${published.css}\n${published.javascript}`);
        ...
        const createdRevision = this.revisions.create({
            ...
            revisionName,
            changeNote,
            sourceHash: (0, project_scraper_code_repository_1.hashSource)(`${input.html}\n${input.css}\n${input.javascript}`),
            validationStatus: "valid",
            createdBy: actor.userId,
            message,
            metadata: { storageRevisionId: revisionId },
        ...
        const versionTag = typeof createdRevision.versionNumber === "number" && createdRevision.versionNumber > 0
            ? `v${createdRevision.versionNumber}`
            : null;
        this.recordProjectActivity(project, {
            type: "developer-tools.display-published",
            message: versionTag && revisionName
    ...
    createDisplay(projectSlug, input) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        ...
        if (normalizedDescription.length > MAX_DISPLAY_DESCRIPTION_LENGTH) {
            throw new Error(`Display description must be ${MAX_DISPLAY_DESCRIPTION_LENGTH} characters or fewer.`);
        }
        const refreshRateMs = (0, refresh_rate_1.normalizeDisplayRefreshRateMs)(5000);
        if (!(0, refresh_rate_1.isAllowedDisplayRefreshRateMs)(refreshRateMs)) {
            throw new Error("Unsupported display refresh rate.");
        }
        const displaySize = (0, display_size_1.normalizeDisplaySize)(display_size_1.DEFAULT_DISPLAY_WIDTH, display_size_1.DEFAULT_DISPLAY_HEIGHT);
        const baseSlug = (0, path_utils_1.sanitizeDisplaySlug)(input.slug?.trim() || (0, display_utils_1.generateDisplaySlugFromName)(trimmedName));
        const slug = this.resolveUniqueDisplaySlug(project.id, baseSlug);
        const slugIssues = (0, display_code_validation_service_1.validateDisplaySlug)(slug);
        if (slugIssues.some((issue) => issue.severity === "error")) {
            throw new Error(slugIssues[0]?.message ?? "Invalid display slug.");
        }
        const html = input.html?.trim() || templates_1.BLANK_DISPLAY_HTML;
        const validation = (0, display_code_validation_service_1.validateDisplaySource)({
            html,
            css: "",
            javascript: "",
        ...
        if (!validation.ok) {
            throw new Error(validation.issues.find((issue) => issue.severity === "error")?.message ??
                "Invalid display HTML.");
        }
        const isStandaloneDocument = /^<!doctype html/i.test(html) || /^<html[\s>]/i.test(html);
        let createdDisplayId = null;
        try {
    ...
    reorderDisplays(projectSlug, displayKeys) {
        const project = this.requireProject(projectSlug);
        this.assertDeveloperToolsAccess(project.id);
        const known = new Set(this.displays.listByProject(project.id).map((row) => row.displayKey));
    ...
    renameDisplay(projectSlug, displayId, input) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const trimmedName = input.name.trim();
        if (!trimmedName) {
    ...
    updateDisplayDetails(projectSlug, displayIdOrKey, input) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const display = this.resolveDisplayRecord(project.id, displayIdOrKey);
        if (!display) {
    ...
    resolveUniqueDisplaySlug(projectId, baseSlug) {
    ...
    resolveUniqueCopyName(projectId, sourceName) {
        const baseName = `${sourceName.trim()} Copy`.trim();
        const existingNames = new Set(this.displays.listByProject(projectId).map((row) => row.name.trim().toLowerCase()));
    ...
    resolveDuplicateSlug(projectId, sourceSlug, copyName) {
        const preferredBase = (0, path_utils_1.sanitizeDisplaySlug)(`${sourceSlug}-copy`);
        const nameBase = (0, path_utils_1.sanitizeDisplaySlug)((0, display_utils_1.generateDisplaySlugFromName)(copyName));
        const baseSlug = this.displayCode.getBySlug(projectId, preferredBase)
            ? nameBase
            : preferredBase;
        return this.resolveUniqueDisplaySlug(projectId, baseSlug);
    }
    buildDisplayViewerUrl(projectId, slug) {
    ...
    duplicateDisplay(projectSlug, displayId, input) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        ...
        const duplicateName = this.resolveUniqueCopyName(project.id, input.name?.trim() || sourceDisplay.name);
        ...
        const published = this.storage.readDisplayPublished(project.id, displayId) ??
            {
                html: sourceCode.draftHtml ?? templates_1.BLANK_DISPLAY_HTML,
                css: sourceCode.draftCss ?? templates_1.BLANK_DISPLAY_CSS,
                javascript: sourceCode.draftJavascript ?? templates_1.BLANK_DISPLAY_JAVASCRIPT,
                metadata: {},
            };
        const display = this.displays.upsert({
            projectId: project.id,
    ...
    setDisplayEnabled(projectSlug, displayId, enabled) {
        const project = this.requireProject(projectSlug);
        this.assertDeveloperToolsAccess(project.id);
    ...
    archiveDisplay(projectSlug, displayId) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const display = this.displays.getById(displayId);
        if (!display || display.projectId !== project.id) {
    ...
    unarchiveDisplay(projectSlug, displayId) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const display = this.displays.getById(displayId);
        if (!display || display.projectId !== project.id) {
    ...
    deleteDisplay(projectSlug, displayId) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        ...
        const code = this.displayCode.getByDisplayId(displayId);
        if (PROTECTED_DISPLAY_KEYS.has(display.displayKey)) {
            throw new Error("Built-in displays cannot be permanently deleted. Archive or disable the display instead.");
        }
    ...
    restoreScraperRevision(projectSlug, revisionId, input) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        ...
        return this.publishScraper(projectSlug, {
            ...
            revisionName: (0, revision_labels_1.normalizeRevisionName)(input?.revisionName) ?? fallbackName,
            changeNote: (0, revision_labels_1.normalizeChangeNote)(input?.changeNote),
    ...
    restoreDisplayRevision(projectSlug, displayId, revisionId, input) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const revision = this.revisions.getById(revisionId);
        if (!revision ||
    ...
    setActiveDisplayRevision(projectSlug, displayId, revisionId) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const code = this.displayCode.getByDisplayId(displayId);
        if (!code || code.projectId !== project.id) {
        ...
        const versionTag = typeof revision.versionNumber === "number" && revision.versionNumber > 0
    ...
    renameDisplayRevision(projectSlug, displayId, revisionId, input) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const revision = this.revisions.getById(revisionId);
        if (!revision ||
        ...
        const nextName = (0, revision_labels_1.normalizeRevisionName)(input.revisionName);
        if (!nextName) {
    ...
    deleteDisplayRevision(projectSlug, displayId, revisionId) {
        const project = this.requireProject(projectSlug);
        const actor = this.assertDeveloperToolsAccess(project.id);
        const code = this.displayCode.getByDisplayId(displayId);
        if (!code) {
    ...
    resolveDisplayViewer(projectId, slug, options) {
        ...
        if (code.archived) {
            return { status: "disabled", displayName: display.name };
        }
        ...
        if (!previewMode && !display.enabled && outputMode) {
            return {
                status: "ok",
                html: (0, templates_1.buildTransparentOutputShellHtml)({
        ...
        const adapterContext = {
            ...
            displayKey: display.displayKey,
            settings,
            runtimeAdapterKey: (0, html_display_runtime_adapters_1.resolveDisplayRuntimeAdapterKey)({
                projectId,
                projectSlug: project.slug,
                displayId: code.displayId,
                slug: code.slug,
        ...
        const servedHtml = (0, html_display_runtime_adapters_1.applyHtmlDisplayRuntimeAdapters)(published.html, adapterContext);
        const standalone = published.metadata &&
            typeof published.metadata === "object" &&
            published.metadata.standaloneDocument === true;
        if (standalone ||
            ...
            const pollIntervalMs = (0, refresh_rate_1.normalizeDisplayRefreshRateMs)(display.refreshRateMs);
            return {
                status: "ok",
                html: (0, templates_1.wrapStandaloneDisplayHtml)({
                    html: servedHtml,
                    dataUrl,
                    displayInfo: {
                        projectId,
        ...
        const localApiBase = this.resolveLocalApiBase();
        const pollIntervalMs = (0, refresh_rate_1.normalizeDisplayRefreshRateMs)(display.refreshRateMs);
        const html = (0, templates_1.buildDisplayDocument)({
            html: servedHtml,
            css: published.css,
            javascript: published.javascript,
            title: display.name,
    ...
    getDisplayViewerEnabled(projectId, slug) {
        const code = this.displayCode.getBySlug(projectId, slug);
        if (!code) {
            throw new Error("Display not found.");
        }
    ...
    getDisplayViewerMeta(projectId, slug) {
        const code = this.displayCode.getBySlug(projectId, slug);
        if (!code) {
            throw new Error("Display not found.");
        }
        ...
        const settings = display.settings && typeof display.settings === "object" ? display.settings : {};
        const runtimeAdapterKey = (0, html_display_runtime_adapters_1.resolveDisplayRuntimeAdapterKey)({
            projectId,
            projectSlug,
            displayId: code.displayId,
            slug: code.slug,
    ...
    buildDisplayViewerPath(projectId, slug) {
        return `/display/${encodeURIComponent(projectId)}/${encodeURIComponent(slug)}`;
    }
    buildDisplayDataUrl(projectId, slug, preview = false) {
    ...
    getRevisionSource(projectSlug, revisionId) {
        const project = this.requireProject(projectSlug);
        this.assertDeveloperToolsAccess(project.id);
        const revision = this.revisions.getById(revisionId);
        ...
        if (revision.resourceType === "display") {
            const bundle = this.storage.readDisplayRevision(project.id, revision.resourceId, storageRevisionId);
            if (!bundle) {
                throw new Error("Revision source files were not found.");
            }
            return {
    ...
    listRevisions(projectSlug, input) {
        const project = this.requireProject(projectSlug);
        this.assertDeveloperToolsAccess(project.id);
        const activeRevisionId = input.resourceType === "scraper"
        ...
        return this.revisions
            ...
            createdByName: typeof revision.metadata.createdByName === "string"
                ? revision.metadata.createdByName
                : null,
            isActive: revision.id === activeRevisionId,
        }));
    }
    resolveRevisionStorageId(revision) {
    ...
    listValidationLogs(projectSlug) {
        const project = this.requireProject(projectSlug);
        this.assertDeveloperToolsAccess(project.id);
        return this.validationLogs.listByProject(project.id);
    }
    recordProjectActivity(project, input, actor) {
        this.recordActivity?.({
            ...input,
            actor: actor
    ...
    async restartProjectScraper(projectId) {
...
exports.DeveloperToolsService = DeveloperToolsService;
