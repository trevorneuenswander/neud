...
exports.handleDeveloperToolsRoute = handleDeveloperToolsRoute;
const local_api_errors_1 = require("../auth/local-api-errors");
async function handleDeveloperToolsRoute(ctx) {
    const match = ctx.url.pathname.match(/^\/api\/projects\/([^/]+)\/developer-tools(?:\/(.+))?$/);
    if (!match) {
        return false;
    }
    ...
    try {
        ...
        if (remainder === "scraper/validate" && ctx.request.method === "POST") {
            const body = await ctx.readJsonBody(ctx.request);
            ctx.sendJson(ctx.response, 200, {
                validation: ctx.developerTools.validateScraperDraft(slug, typeof body.source === "string" ? body.source : undefined),
            });
            return true;
        }
        if (remainder === "scraper/publish" && ctx.request.method === "POST") {
            ...
            const result = await ctx.developerTools.publishScraper(slug, {
                ...
                revisionName: typeof body.revisionName === "string" ? body.revisionName : undefined,
        ...
        if (remainder === "displays/reorder" && ctx.request.method === "POST") {
            ...
            const displayKeys = Array.isArray(body.displayKeys)
                ? body.displayKeys.filter((entry) => typeof entry === "string")
                : Array.isArray(body.displayIds)
                    ? body.displayIds.filter((entry) => typeof entry === "string")
                    : [];
        ...
        if (displayMatch) {
            ...
            if (action === "enabled" && ctx.request.method === "PATCH") {
                const body = await ctx.readJsonBody(ctx.request);
                ctx.sendJson(ctx.response, 200, {
                    display: ctx.developerTools.setDisplayEnabled(slug, displayId, body.enabled === true),
                });
                return true;
            }
        ...
        if (remainder === "scraper/restore" && ctx.request.method === "POST") {
            const body = await ctx.readJsonBody(ctx.request);
            const result = await ctx.developerTools.restoreScraperRevision(slug, String(body.revisionId ?? ""), {
                message: typeof body.message === "string" ? body.message : undefined,
                revisionName: typeof body.revisionName === "string" ? body.revisionName : undefined,
                changeNote: typeof body.changeNote === "string" ? body.changeNote : undefined,
            });
        ...
        if (displayRestoreMatch && ctx.request.method === "POST") {
            const body = await ctx.readJsonBody(ctx.request);
            ctx.sendJson(ctx.response, 200, {
                display: ctx.developerTools.restoreDisplayRevision(slug, displayRestoreMatch[1], String(body.revisionId ?? ""), {
                    message: typeof body.message === "string" ? body.message : undefined,
                    revisionName: typeof body.revisionName === "string" ? body.revisionName : undefined,
                    changeNote: typeof body.changeNote === "string" ? body.changeNote : undefined,
                }),
        ...
        if (displayActivateMatch && ctx.request.method === "POST") {
            const body = await ctx.readJsonBody(ctx.request);
            ctx.sendJson(ctx.response, 200, {
                display: ctx.developerTools.setActiveDisplayRevision(slug, displayActivateMatch[1], String(body.revisionId ?? "")),
            });
            return true;
        }
        const displayRevisionRenameMatch = remainder.match(/^displays\/([^/]+)\/revisions\/([^/]+)\/description$/);
        if (displayRevisionRenameMatch && ctx.request.method === "PATCH") {
            const body = await ctx.readJsonBody(ctx.request);
            ctx.sendJson(ctx.response, 200, ctx.developerTools.renameDisplayRevision(slug, displayRevisionRenameMatch[1], displayRevisionRenameMatch[2], { revisionName: String(body.revisionName ?? "") }));
            return true;
        }
        const displayRevisionDeleteMatch = remainder.match(/^displays\/([^/]+)\/revisions\/([^/]+)$/);
        if (displayRevisionDeleteMatch && ctx.request.method === "DELETE") {
            ctx.sendJson(ctx.response, 200, {
                result: ctx.developerTools.deleteDisplayRevision(slug, displayRevisionDeleteMatch[1], displayRevisionDeleteMatch[2]),
            });
            return true;
        }
        ...
        ctx.sendJson(ctx.response, 404, (0, local_api_errors_1.localApiErrorResponse)(local_api_errors_1.LOCAL_API_ERROR_CODES.PROJECT_NOT_FOUND, "Developer Tools route not found."));
        return true;
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "Developer Tools request failed.";
        ...
        const code = status === 401
            ...
            : status === 403
                ? local_api_errors_1.LOCAL_API_ERROR_CODES.FORBIDDEN
                : status === 404
                    ? local_api_errors_1.LOCAL_API_ERROR_CODES.PROJECT_NOT_FOUND
                    : undefined;
        ctx.sendJson(ctx.response, status, code ? (0, local_api_errors_1.localApiErrorResponse)(code, message) : { error: message });
        return true;
    }
}
