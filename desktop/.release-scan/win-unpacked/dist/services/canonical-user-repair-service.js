...
exports.repairCanonicalAccounts = repairCanonicalAccounts;
...
function repairCanonicalAccounts(input) {
    ...
    if (ownerCandidates.length > 1) {
        return fail(ownerCandidates.map((user) => user.id), `Ambiguous owner records for ${default_owner_email_1.DEFAULT_OWNER_EMAIL}.`);
    }
    ...
    if (hildrethCandidates.length > 1) {
        return fail(hildrethCandidates.map((user) => user.id), `Ambiguous Hildreth admin records for ${default_owner_email_1.HILDRETH_ADMIN_EMAIL}.`);
    }
    ...
    if (ownerUser &&
        ...
        return fail([ownerUser.id, hildrethUser.id], `Owner and Hildreth admin share the same Supabase UUID (${ownerUser.supabaseUserId}).`);
    }
    ...
    for (const user of input.users.listAll()) {
        ...
        else if ((0, default_owner_email_1.isLegacyPlaceholderOwnerEmail)(email) && user.isActive) {
            input.users.updateIdentity({
                id: user.id,
                platformRole: "user",
                isActive: false,
...
function fail(ambiguousUserIds, error) {
    return {
