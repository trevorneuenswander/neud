...
exports.validateImportPackage = validateImportPackage;
exports.countImportTotals = countImportTotals;
...
function validateImportPackage(pkg) {
    const errors = [];
    const warnings = [];
    if (pkg.version !== import_types_1.NEUD_IMPORT_VERSION) {
        errors.push({
            path: "version",
    ...
    const scraperSourceIds = new Set();
    for (const [index, project] of pkg.projects.entries()) {
        ...
        for (const [engineIndex, engine] of project.engines.entries()) {
            validateEngine(engine, `projects[${index}].engines[${engineIndex}]`, errors, warnings);
            ...
            for (const [sourceIndex, source] of engine.scraperSources.entries()) {
                validateScraperSource(source, `projects[${index}].engines[${engineIndex}].scraperSources[${sourceIndex}]`, errors, warnings);
            ...
            if (engine.latestSnapshot) {
                validateSnapshot(engine.latestSnapshot, engine.id, `projects[${index}].engines[${engineIndex}].latestSnapshot`, errors, warnings);
            }
        }
...
function validateProject(project, path, errors, warnings) {
...
function validateEngine(engine, path, errors, warnings) {
    ...
    if (!import_types_1.SUPPORTED_ENGINE_TYPES.includes(engine.engineType)) {
        warnings.push({
            path: `${path}.engineType`,
            message: `Engine type "${engine.engineType}" is not fully supported locally.`,
        });
    }
    if (!import_types_1.SUPPORTED_DESIRED_STATES.includes(engine.desiredState)) {
        errors.push({
            path: `${path}.desiredState`,
            message: `Unsupported desired state "${engine.desiredState}".`,
        });
    }
    if (!import_types_1.SUPPORTED_EXECUTION_MODES.includes(engine.executionMode)) {
        errors.push({
    ...
    if (engine.settings) {
        if (!Number.isFinite(engine.settings.pollIntervalMs) ||
...
function validateScraperSource(source, path, errors, warnings) {
...
function validateSnapshot(snapshot, engineId, path, errors, warnings) {
...
function countImportTotals(pkg) {
    let engines = 0;
