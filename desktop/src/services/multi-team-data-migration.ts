...
export function runMultiTeamDataMigration(db: LocalDatabase): void {
  const marker = db
    .prepare("SELECT value_json FROM app_settings WHERE key = ?")
    .get(MULTI_TEAM_MIGRATION_KEY) as { value_json: string } | undefined;
  ...
  db.transaction(() => {
    ...
    ).run(MULTI_TEAM_MIGRATION_KEY, JSON.stringify("done"));
  });

  console.info("[multi-team-migration] Project team assignments migration completed.");
}

function copyLegacyProjectTeams(db: LocalDatabase): void {
  const projects = db
  ...
  for (const project of projects) {
    const existing = db
      ...
      .get(project.id, project.team_id) as { id: string } | undefined;
    if (existing) {
      continue;
    }
    ...
    ).run(randomUUID(), project.id, project.team_id);
  }
}

function rebuildTeamScopedProjectMemberships(db: LocalDatabase): void {
  if (tableHasColumn(db, "project_memberships", "team_id")) {
    return;
  }

  const legacyRows = db.prepare("SELECT * FROM project_memberships").all() as Array<{
    id: string;
    project_id: string;
    user_id: string;
    access_role: string;
  ...
  const insert = db.prepare(
    `INSERT OR IGNORE INTO project_memberships_v2 (
  ...
  for (const row of legacyRows) {
    ...
    if (!teamId) {
      console.warn(
        `[multi-team-migration] Skipped unscoped membership project=${row.project_id} user=${row.user_id}`,
      );
      continue;
    }
    insert.run(
...
function resolveMembershipTeamId(
  ...
  const sharedTeam = db
    ...
    .get(projectId, userId) as { team_id: string } | undefined;
  ...
  const projectTeam = db
    ...
    .get(projectId) as { team_id: string } | undefined;

  if (projectTeam?.team_id) {
    const userOnTeam = db
      .prepare(
        "SELECT team_id FROM team_memberships WHERE team_id = ? AND user_id = ? AND is_active = 1",
      )
      .get(projectTeam.team_id, userId) as { team_id: string } | undefined;
    if (userOnTeam) {
