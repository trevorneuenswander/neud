import { isUsablePhotoUrl } from "../displays/display-utils";
import type { OfflineAuctionLot, OfflinePhotoReference } from "./offline-auction-types";

export type NormalizedLotThumbnail = {
  url: string;
  alt: string;
  source: "local" | "remote";
};
...
export function photoCanonicalKey(
  photo: string | OfflinePhotoReference,
  buildLocalAssetUrl?: (relativePath: string) => string | null,
): string | null {
...
function resolvePhotoReference(
  photo: string | OfflinePhotoReference,
  buildLocalAssetUrl?: (relativePath: string) => string | null,
  ...
  const localPath =
    typeof photo.relativePath === "string" && photo.relativePath.trim()
...
export function normalizeLotThumbnailPhotos(
  lot: OfflineAuctionLot | null | undefined,
  buildLocalAssetUrl?: (relativePath: string) => string | null,
): NormalizedLotThumbnail[] {
  return resolveLotPreviewPhotos(lot, buildLocalAssetUrl);
}

export function resolveLotPreviewPhotos(
  lot: OfflineAuctionLot | null | undefined,
  buildLocalAssetUrl?: (relativePath: string) => string | null,
): NormalizedLotThumbnail[] {
  if (!lot) return [];
  ...
  const ordered: NormalizedLotThumbnail[] = [];
  ...
  const sources: Array<string | OfflinePhotoReference> = [];
  ...
  for (const entry of sources) {
    ...
    if (resolved.localUrl) {
      const displayUrl =
        resolved.localUrl.startsWith("/api/offline-assets/") || /^https?:\/\//i.test(resolved.localUrl)
          ? resolved.localUrl
...
export function findLotInAuctionExport(
  lots: OfflineAuctionLot[] | null | undefined,
  lotNumber: string,
): OfflineAuctionLot | null {
  if (!lots || !lotNumber.trim()) return null;
  const normalized = lotNumber.replace(/^lot\s+/i, "").trim().toLowerCase();
  return (
    lots.find((lot) => readLotNumber(lot).toLowerCase() === normalized) ??
