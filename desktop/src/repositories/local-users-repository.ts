...
import type { LocalUserRecord, PlatformRole } from "../services/access-types";
...
type LocalUserRow = {
  ...
  full_name: string;
  phone: string | null;
  profile_team: string | null;
  supabase_user_id: string | null;
  platform_role: string;
  is_active: number;
  supabase_account_available?: number;
  last_supabase_sync_at?: string | null;
  invitation_status: string | null;
  created_at: string;
  updated_at: string;
};

function mapUser(row: LocalUserRow): LocalUserRecord {
  return {
    id: row.id,
    email: row.email,
...
export class LocalUsersRepository {
  ...
  countByPlatformRole(platformRole: PlatformRole): number {
    const row = this.db
      .prepare("SELECT COUNT(*) AS count FROM local_users WHERE platform_role = ? AND is_active = 1")
      .get(platformRole) as { count: number };
    return row.count;
  }

  getById(userId: string): LocalUserRecord | null {
    const row = this.db
  ...
  getByEmail(email: string): LocalUserRecord | null {
    const row = this.db
      .prepare("SELECT * FROM local_users WHERE email = ? COLLATE NOCASE")
      .get(email.trim()) as LocalUserRow | undefined;
    return row ? mapUser(row) : null;
  }

  getBySupabaseUserId(supabaseUserId: string): LocalUserRecord | null {
    const row = this.db
      .prepare("SELECT * FROM local_users WHERE supabase_user_id = ? OR id = ?")
      .get(supabaseUserId, supabaseUserId) as LocalUserRow | undefined;
    return row ? mapUser(row) : null;
  }

  listBySupabaseUserIds(supabaseUserIds: string[]): LocalUserRecord[] {
    ...
    const rows = this.db
      .prepare(
        `SELECT * FROM local_users WHERE supabase_user_id IN (${placeholders}) OR id IN (${placeholders})`,
      )
      .all(...supabaseUserIds, ...supabaseUserIds) as LocalUserRow[];
    return rows.map(mapUser);
  }

  listByEmail(email: string): LocalUserRecord[] {
    const normalized = email.trim().toLowerCase();
    const rows = this.db
      .prepare("SELECT * FROM local_users WHERE email = ? COLLATE NOCASE")
      .all(normalized) as LocalUserRow[];
    return rows.map(mapUser);
  }
  ...
  applySupabaseSync(input: {
    ...
    fullName: string;
    syncedAt: string;
    linkToExistingId?: string | null;
    preservePlatformRole?: PlatformRole;
    preserveIsActive?: boolean;
  }): { user: LocalUserRecord; created: boolean; linked: boolean; updated: boolean } {
    const normalizedEmail = input.email.trim().toLowerCase();
    const now = input.syncedAt;
    let existing =
      this.getBySupabaseUserId(input.supabaseUserId) ??
    ...
    if (existing) {
      ...
      this.db
        ...
        .run(
          ...
          now,
          input.preserveIsActive === false ? 0 : existing.isActive ? 1 : 1,
          now,
          existing.id,
        );
  ...
  markSupabaseUnavailableExcept(availableSupabaseUserIds: string[], syncedAt: string): number {
  ...
  deactivateDuplicateByEmail(keepUserId: string, email: string): number {
    const normalized = email.trim().toLowerCase();
    const now = new Date().toISOString();
    const duplicates = this.db
      .prepare("SELECT id FROM local_users WHERE lower(email) = ? AND id != ?")
      .all(normalized, keepUserId) as Array<{ id: string }>;
  ...
  private countSupabaseAvailableLinkedUsers(): number {
    const row = this.db
      .prepare(
        "SELECT COUNT(*) AS count FROM local_users WHERE supabase_user_id IS NOT NULL AND supabase_account_available = 1",
      )
      .get() as { count: number };
    return row.count;
  }

  resolveByAuthUserId(authUserId: string): LocalUserRecord | null {
    return this.getBySupabaseUserId(authUserId);
  }

  listAll(): LocalUserRecord[] {
    const rows = this.db
  ...
  upsert(input: {
    ...
    const now = new Date().toISOString();
    const existing =
      (input.supabaseUserId ? this.getBySupabaseUserId(input.supabaseUserId) : null) ??
    ...
    if (existing) {
      this.db
        ...
        .run(
          ...
          input.invitationStatus ?? existing.invitationStatus,
    ...
    const id = input.id ?? input.supabaseUserId ?? randomUUID();
    this.db
      .prepare(
        `INSERT INTO local_users (
  ...
  updateIdentity(input: {
    ...
    this.db
      ...
      .run(
        ...
        (input.fullName ?? existing.fullName).trim(),
        input.platformRole ?? existing.platformRole,
        input.supabaseUserId === undefined
          ? existing.supabaseUserId
          : input.supabaseUserId,
        input.isActive === undefined ? (existing.isActive ? 1 : 0) : input.isActive ? 1 : 0,
        input.supabaseAccountAvailable === undefined
          ? existing.supabaseAccountAvailable
            ? 1
            : 0
          : input.supabaseAccountAvailable
            ? 1
            : 0,
        input.lastSupabaseSyncAt === undefined
          ? existing.lastSupabaseSyncAt
          : input.lastSupabaseSyncAt,
        now,
        input.id,
      );
  ...
  updateSyncedProfileFields(input: {
    ...
    const nextProfileTeam =
      input.profileTeam === undefined
  ...
  setActive(userId: string, isActive: boolean): LocalUserRecord | null {
    const now = new Date().toISOString();
    this.db
