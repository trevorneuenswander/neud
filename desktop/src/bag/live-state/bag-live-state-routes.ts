...
import type { BagLiveStateService } from "./bag-live-state-service";
import type { BagLiveStateUpdatedEvent } from "./bag-live-state-types";

type BagRouteContext = {
  bagLiveState: BagLiveStateService;
  bagEvents: BagLiveStateEvents;
  baseUrl: string;
  requireAuth: () => void;
  requireControlAccess: () => void;
  getActor: () => { id?: string | null; email?: string | null; name?: string | null };
  notifyLocalControllerStateChanged?: (projectId: string, actionType: string) => void;
  onLotPhotoRemoved?: (projectId: string, photoUrl: string) => void;
  /** @deprecated Use notifyLocalControllerStateChanged */
  notifyLocalControllerCanonicalChanged?: (projectId: string, actionType: string) => void;
...
export async function handleBagRoute(
  request: IncomingMessage,
  response: ServerResponse,
  url: URL,
  ctx: BagRouteContext,
): Promise<boolean> {
  const projectIdMatch = url.pathname.match(
    /^\/api\/projects\/([^/]+)\/bag\/(.+)$/,
  );
  ...
  if (subpath === "controller" && request.method === "GET") {
    sendJson(
      response,
      200,
      ctx.bagLiveState.getControllerInfo(projectId, ctx.baseUrl),
      true,
    );
    return true;
  }
  ...
  if (subpath.startsWith("manual/")) {
    return await handleManualRoute(
      request,
      response,
      subpath.slice("manual/".length),
      projectId,
      ctx,
    );
  }
...
function handleLiveEvents(
  ...
  const send = (event: BagLiveStateUpdatedEvent) => {
...
async function handleManualRoute(
  ...
  try {
    ...
    const status =
      message.includes("Sign in") || message.includes("Authentication")
        ? 401
        : 403;
  ...
  if (subpath === "enter" && request.method === "POST") {
    ...
    if (result.ok) {
      ctx.notifyLocalControllerCanonicalChanged?.(projectId, "enter-manual-mode");
    }
  ...
  if (subpath === "load-lot" && request.method === "POST") {
    ...
    const result = ctx.bagLiveState.loadLotForEditing(
      projectId,
      {
        lotNumber,
        title: title ?? "",
  ...
  if (subpath === "lot/photos/reorder" && request.method === "POST") {
    ...
    const photoUrls = Array.isArray(body.photoUrls)
      ? body.photoUrls.filter((entry): entry is string => typeof entry === "string")
      : [];
    if (!lotNumber) {
      sendJson(response, 400, { error: "lotNumber is required." });
      return true;
    }
    const result = ctx.bagLiveState.updateDraftLotPhotoOrder(
      projectId,
      lotNumber,
      photoUrls,
      actor,
    ...
    if (result.ok) {
      ...
      ctx.notifyLocalControllerStateChanged?.(projectId, "controller.lot.photos-reordered");
    }
    return sendManualResult(response, result);
  }
  ...
  if (subpath === "lot/photos/remove" && request.method === "POST") {
    ...
    const result = ctx.bagLiveState.removeDraftLotPhoto(
      projectId,
      lotNumber,
      photoUrl,
      actor,
    ...
    if (result.ok) {
      ...
      ctx.onLotPhotoRemoved?.(projectId, photoUrl);
      ctx.notifyLocalControllerStateChanged?.(projectId, "controller.lot.photos-removed");
      ctx.notifyLocalControllerCanonicalChanged?.(projectId, "controller.lot.photos-removed");
    }
    return sendManualResult(response, result);
  }
  ...
  if (subpath === "lot/photos/add" && request.method === "POST") {
    ...
    const result = ctx.bagLiveState.addDraftLotPhotos(
      projectId,
      lotNumber,
      photoUrls,
      actor,
    ...
    if (result.ok) {
      ...
      ctx.notifyLocalControllerStateChanged?.(projectId, "controller.lot.photos-added");
    }
    return sendManualResult(response, result);
  }
  ...
  if (subpath === "submit/lot" && request.method === "POST") {
    ...
    if (result.ok) {
      ctx.notifyLocalControllerCanonicalChanged?.(projectId, "submit-lot");
      ctx.recordLog?.({
        level: "info",
        ...
        metadata: {
          lotNumber:
            result.envelope.localControllerSubmitted?.currentLot?.lotNumber ?? null,
        },
      });
    }
    return sendManualResult(response, result);
  ...
  if (subpath === "submit/bid" && request.method === "POST") {
    ...
    if (result.ok) {
      ctx.notifyLocalControllerCanonicalChanged?.(projectId, "submit-bid");
      const bidAmount =
        result.envelope.localControllerSubmitted?.currentLot?.currentBid ??
  ...
  if (subpath === "status" && request.method === "POST") {
    ...
    if (result.ok) {
      ctx.notifyLocalControllerCanonicalChanged?.(projectId, "set-lot-status");
      ctx.recordActivity?.({
  ...
  if (subpath === "status/clear" && request.method === "POST") {
    ...
    if (result.ok) {
      ctx.notifyLocalControllerCanonicalChanged?.(projectId, "clear-lot-status");
      ctx.recordActivity?.({
        type: "controller.lot-status",
...
function sendManualResult(
  response: ServerResponse,
  result: { ok: true; envelope: unknown } | { ok: false; error: string },
): boolean {
  if (!result.ok) {
