(function () {
  ...
  function buildAuctionDisplayView(canonical) {
    ...
    var photos = Array.isArray(ad.photos)
      ? ad.photos
    ...
    return {
      lot: textValue(ad.lot) || formatLotLabel(current.lot),
      title: textValue(ad.title) || textValue(current.title) || "",
      year: textValue(ad.year) || textValue(current.year) || null,
      biddingPrice:
        ...
        null,
      reserveStatus:
        textValue(ad.reserveStatus) || textValue(current.status) || null,
      currencies: Array.isArray(ad.currencies)
  ...
  function normalizeNeudPayload(message) {
    ...
    if (message.current || message.auctionDisplay || message.dataSource) {
      return message;
    }
