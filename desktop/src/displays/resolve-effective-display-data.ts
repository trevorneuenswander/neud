...
export function resolveEffectiveDisplayData(input: {
  ...
  photoOverrides?: import("../bag/live-state/lot-photo-overrides").LotPhotoOverrides | null;
  enrichCanonicalPhotos?: (
    photos: import("./canonical-photo").CanonicalPhotoInput[],
  ) => import("./canonical-photo").CanonicalPhotoInput[];
  ...
  const localControllerState =
    input.source === "local-controller"
      ? resolveLocalControllerDisplayData({
          ...
          enrichCanonicalPhotos: input.enrichCanonicalPhotos,
        })
