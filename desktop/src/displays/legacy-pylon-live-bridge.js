  ...
  function renderNeudPylonData(rawMessage) {
    ...
    if (!source || typeof window.render !== "function") {
      updateLegacyDiagnostics({
        inputReceived: Boolean(payload),
        renderCompleted: false,
        skipReason: !source ? "missing_pylon_source" : "render_unavailable",
      });
      return;
    }
    ...
    updateLegacyDiagnostics({
      inputReceived: true,
      currentLotResolved: Boolean(textValue(auctionDisplay.lot)),
      bidResolved: Boolean(textValue(auctionDisplay.biddingPrice)),
