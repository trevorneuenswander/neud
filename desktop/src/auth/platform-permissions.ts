...
export function canDeleteProject(actor: ActorLike): boolean {
  const role = actorRole(actor);
  return role === "owner" || role === "admin";
}

export function canManageProject(actor: ActorLike): boolean {
  return isPlatformAdministrator(actor);
}
...
export function canOperateProject(actor: ActorLike): boolean {
  const role = actorRole(actor);
  return role === "owner" || role === "admin" || role === "operator";
}

export function canViewProject(actor: ActorLike): boolean {
  const role = actorRole(actor);
  return (
    role === "owner" ||
    role === "admin" ||
