...
export type PublishedProjectPayloadValidation =
  | { ok: true; payload: NeudPublishedProjectPayload }
  | { ok: false; issues: string[] };

export function validatePublishedProjectPayload(
  value: unknown,
): PublishedProjectPayloadValidation {
  const issues: string[] = [];
  ...
  if (value.contractVersion !== NEUD_PUBLISHED_PROJECT_CONTRACT_VERSION) {
  ...
  if (!isRecord(value.source)) {
    ...
    if (
      value.source.mode !== "webpage-scraper" &&
  ...
  return { ok: true, payload: value as unknown as NeudPublishedProjectPayload };
}
